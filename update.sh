...
update_x-ui() {
    ...
    ${curl_bin} -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/skvarovski/3x-ui/main/x-ui.sh >/dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        echo -e "${yellow}Trying to fetch x-ui with IPv4...${plain}"
        ${curl_bin} -4fLRo /usr/bin/x-ui https://raw.githubusercontent.com/skvarovski/3x-ui/main/x-ui.sh >/dev/null 2>&1
        if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        ...
        ${curl_bin} -fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/skvarovski/3x-ui/main/x-ui.rc >/dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            ${curl_bin} -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/skvarovski/3x-ui/main/x-ui.rc >/dev/null 2>&1
            if [[ $? -ne 0 ]]; then
        ...
        if [ -f "x-ui.service" ]; then
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/skvarovski/3x-ui/main/x-ui.service.debian >/dev/null 2>&1
                    ;;
                    arch | manjaro | parch)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/skvarovski/3x-ui/main/x-ui.service.arch >/dev/null 2>&1
                    ;;
                    *)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/skvarovski/3x-ui/main/x-ui.service.rhel >/dev/null 2>&1
                    ;;
                esac
